        ...
        <template v-for="op in [{ label: 'Yes', value: true }, { label: 'No', value: false }] " :key="op">
          <template v-if="isEditor && field">
            <div class="flex items-center gap-1 hover:bg-sky-50">
              <input type="radio" :name="`option-${field.property ?? field.id}-${op.value}`" v-model="value"
                :value="op.value" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-600" />
              <InlineContent v-model="op.label" />
            </div>
          </template>
          <template v-else>
            <div class="flex items-center gap-1.5">
              <RadioButton v-model="value" :inputId="`option-${field.property ?? field.id}-${op.value}`"
                :name="`fld-${field.id}`" :value="op.value" />
