...
const styles = {
  base: [
    // Base
    'relative isolate inline-flex items-center justify-center gap-x-2 rounded border text-base/6 font-semibold',
...
const { color = 'dark/zinc', outline, plain, is = 'button' } = defineProps<ButtonProps>()
const classes = clsx(
  styles.base,
...
<template>
  <Component :is="is" :class="cn(classes, $attrs.class as any)">
    <slot />
  </Component>
</template>
<!--

export function Button({ color = 'dark/zinc', outline, plain, is }: ButtonProps, { slots, attrs }) {
  const classes = clsx(
    attrs.class,
    styles.base,
    outline ? styles.outline : plain ? styles.plain : clsx(styles.solid, styles.colors[color])
  ...
  return () => h(is, { class: cn(classes, attrs.class) }, slots.default())
}
 -->
